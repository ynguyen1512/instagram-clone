...
const Timeline = () => {
  ...
  const addNewPost = () => {
    const newPost = {
      user: "newUser", // This is an example, replace with the desired username
      postImage: "path_to_new_image", // Replace with the path or URL to the new post image
      like: 0,
      timestamp: "Just now", // This is a placeholder. You can replace it with a function to get the current timestamp.
    };
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth <= 520) {
        // For small screens, show only 4 storyUser
        setStoryUserToShow(4);
      } else if (window.innerWidth <= 768) {
        setStoryUserToShow(7);
      } else if (window.innerWidth <= 1200) {
        setStoryUserToShow(10);
      } else {
